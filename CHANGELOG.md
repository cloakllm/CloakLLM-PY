...
### Fixed
- **SECURITY: credit-card detection was missing entire issuer ranges, and partially leaking them into the audit log.** The BIN list stopped at Visa, 5-series Mastercard, Amex and Discover, so Luhn-valid cards on three live ranges were not detected at all: **Mastercard 2-series** (2221-2720, issued since 2017), **JCB** (3528-3589) and **UnionPay** (62, the largest network in the world by volume). Discover 644-649, Diners Club and 17-19 digit UnionPay were missing too. Identical in both SDKs, so no cross-SDK divergence -- the same hole everywhere, inherited by `cloakllm-mcp`, the Guard extension and the MCP gateway. **And it was worse than non-detection:** on the normal spaced forms, `PHONE` claimed part of each card and the remaining digits were written to the audit log verbatim -- four partial leaks, the exact class of failure v0.11.2 existed to remove, reopened on four issuer ranges. Running the extended corpus against the previous release shows character-level scrub falling from 97.4% to 84.6%.
- **No Luhn checksum, so numbers that merely looked like cards were reported as cards.** `4111111111111112` -- the standard test Visa with a deliberately broken check digit -- was flagged. Luhn now gates every match. Rejection happens in `detect()` rather than in the regex, which leaves the span uncovered, so a rejected candidate is not silently relabelled as a `PHONE` instead. Prefix **and** checksum, never either alone: Luhn by itself admits one in ten random digit runs, which on a developer's order ids and timestamps would be a false-positive engine. Maestro (50, 56-69, 12-19 digits) remains deliberately uncovered -- its range overlaps most other issuers and a great deal of ordinary text -- and a test asserts that gap so it stays a decision.
- **The ReDoS safety check measured wall clock, and failing it silently disables a detection category.** `_test_regex_safety` rejects a pattern that takes 100ms on a probe corpus, and a rejected pattern is *skipped* -- detection for that category switched off with only a warning. Measuring wall clock meant measuring whatever else the machine was doing. **EMAIL sat at roughly 6x of headroom where every other pattern had 100x-1300x**, so a busy machine could stop detecting email addresses; running a second test suite concurrently was enough to reproduce it. Now measured in CPU time, which is what catastrophic backtracking actually is. The deeper question -- that this guard fails *open* at all -- is deliberately untouched, since changing it alters failure semantics for existing deployments.
- **The cross-SDK whole-float guard was a 1-in-9 flake.** It asserted `".0" not in raw` over an entire serialised audit entry, which also matches the microsecond field of any timestamp whose fraction begins with a zero. Measured at 11.7% over 300 runs, every failure a timestamp and not one a real float -- on the test guarding the invariant that made genuine chains read as tampered. Now checked by parsing the entry and walking for floats holding whole values, which reads the property directly and cannot confuse a number with a date.

### Added
- **A false-positive corpus, measured as a number.** The project measured character-level scrub -- recall -- and had no false-positive measurement of any kind, which is why a missing checksum was invisible to every gate including AUDIT-14. `tests/test_detection_v0123.py` states it on a corpus of what a developer has in a chat window all day (ISBNs, order ids, IMEIs, millisecond timestamps, UUIDs, git SHAs, tracking numbers): **0 of 13 credit-card false positives**, against 2 before this release. Note the scope carefully -- that number is *credit-card* false positives measured *regex-only*. Counting every category, the same corpus scores **1 of 13** regex-only (a build number read as a `PHONE`) and **3 of 13** in the default config, which has NER on (a git SHA read as a `PERSON`, a bare word as an `ORG`). Those are pre-existing and untouched here; they are newly *visible* because the corpus now exists. Measuring them in the default config -- the one users actually get -- is the next iteration.
- **`detector.luhn_valid()`**, the checksum used by the card matcher.
- **Issuer coverage in the hard corpus.** `benchmarks/corpus_hard.json` held only Visa, 5-series Mastercard and Amex, so the recall benchmark *could never have caught this*. It now carries one Luhn-valid number per issuer range, verified by running the new corpus against the old code: scrub 97.4% -> 84.6%, partial leaks 0 -> 4. Closing the measurement gap matters more than the one-off fix.
- The cross-SDK detection differential (`benchmarks/xsdk_detection_diff.py`) covers the new ranges, the range edges and the rejected shapes. 177 inputs, 0 divergences.
...
### Fixed
- **StreamDesanitizer fidelity** (LOW — cosmetic, no PII leak): streamed desanitize now byte-equals batch desanitize for *every* chunking. Previously, when the user's own text contained a literal `[CATEGORY_N]`-style token, sanitize escaped it to fullwidth brackets (`［…］`) and the stream emitted the escaped sequence in fragments, so `_unescape` never saw a whole `［…］` to convert back — leaking the fullwidth form instead of restoring `[…]`. Token-injection prevention held in both paths (the escaped literal is never replaced with PII), so this was fidelity-only. The stream now buffers fullwidth brackets across chunk boundaries too. Regression-guarded by a streaming-vs-batch fuzz (`tests/test_stream_fuzz.py`). Rides the next version bump.
