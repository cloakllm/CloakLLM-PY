...
### Added
- **A false-positive corpus, measured as a number.** The project measured character-level scrub -- recall -- and had no false-positive measurement of any kind, which is why a missing checksum was invisible to every gate including AUDIT-14. `tests/test_detection_v0123.py` states it: **0 of 13** on a corpus of what a developer has in a chat window all day (ISBNs, order ids, IMEIs, millisecond timestamps, UUIDs, git SHAs, tracking numbers).
